...
class FerretOtInterface {
 public:
  ...
  // Raw random correlated OT.
  //
  // Receiver choices are generated by RCOT and returned to the caller.
  // No correlation-breaking hash is applied to the returned 128-bit blocks.
  virtual void SendRCOT(absl::Span<uint128_t> output) = 0;
  virtual void RecvRCOT(absl::Span<uint128_t> output,
                        absl::Span<uint8_t> choices) = 0;

  // RCOT correlation, valid on the sender side.
  virtual uint128_t GetDelta() const = 0;
